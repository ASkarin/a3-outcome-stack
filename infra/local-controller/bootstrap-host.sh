...
config_root=/etc/a3-outcome-stack
runtime_root=/run/a3-outcome-stack
dropin=/etc/ssh/sshd_config.d/60-a3-local-hardening.conf
rollback_unit=a3-local-security-rollback
...
install_base() {
    ...
    hostnamectl set-hostname a3-local
    apt-get update
    ...
    install -d -m 0755 -o root -g root /opt/a3/python
    UV_PYTHON_INSTALL_DIR=/opt/a3/python uv python install 3.12.13
}
...
install_security() {
    ...
    [[ "${A3_CONFIRM_CONSOLE:-}" == "YES" ]] || \
        fail "set A3_CONFIRM_CONSOLE=YES only with physical-console recovery available"
    ...
    systemd-run --quiet --unit="${rollback_instance}" --on-active=10m \
        /usr/local/sbin/a3-local-security-rollback "${backup}"
    ...
    ufw allow in on tailscale0 proto tcp to any port "${ssh_port}" comment 'A3 Tailscale SSH'
    ufw --force enable
...
confirm_security() {
    ...
    [[ "${rollback_instance}" =~ ^a3-local-security-rollback-[0-9]{8}T[0-9]{6}Z$ ]] || \
...
case "${1:-}" in
    ...
    check) exec bash "${script_dir}/a3-local-doctor.sh" ;;
