...
def validate_hardware_ready(
    ...
    if (
        not isinstance(calibration.get("robot_serial"), str)
        or not calibration["robot_serial"]
    ):
    ...
    if (
        safety.get("approval_status") != "frozen"
        or safety.get("simulation_only") is not False
    ):
    ...
    if (
        not isinstance(safety.get("workspace_limits"), Mapping)
        or not safety["workspace_limits"]
    ):
        raise ValidationError("frozen safety configuration requires workspace_limits")
    lower = _finite_list(
        safety.get("joint_position_lower_rad"), "joint_position_lower_rad"
    )
    upper = _finite_list(
        safety.get("joint_position_upper_rad"), "joint_position_upper_rad"
    )
...
class A3SdkBackend:
    """SDK mapping verified with fakes; hardware_verified stays false in stage 1A."""
    ...
    def observation_features(self) -> Mapping[str, Any]:
        return observation_features(self.config.get("camera_features", {}))
    ...
    def _vendor_kwargs(self) -> dict[str, Any]:
        ...
        try:
            frozen = validate_hardware_ready(self.calibration, self.safety)
        except (StateConflict, ValidationError):
            return kwargs
        kwargs.update(
            {
                "joint_directions": {
                    index + 1: value for index, value in enumerate(frozen["directions"])
                },
                "joint_offsets": {
                    index + 1: value for index, value in enumerate(frozen["offsets"])
                },
                "joint_limits": {
    ...
    def connect(self) -> None:
        ...
        self._vendor = self._factory()(**self._vendor_kwargs())
    ...
    def disable(self) -> None:
        if (
            self._connected
            and self._vendor is not None
            and not self._vendor.DisableArm()
        ):
    ...
    def get_observation(self) -> Observation:
        ...
        vendor_state = getattr(
            getattr(self._vendor, "arm_state", None), "name", "UNKNOWN"
        )
        ...
        return Observation(
            ...
            tcp_orientation_xyzw=rpy_to_quaternion_xyzw(
                end_pose.rx, end_pose.ry, end_pose.rz
            ),
            status=RobotStatus(
    ...
    def emergency_stop(self) -> None:
        if (
            not self._connected
            or self._vendor is None
            or not self._vendor.EmergencyStop()
        ):
